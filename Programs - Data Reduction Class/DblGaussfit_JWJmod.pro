...
PRO DBLGAUSS_FUNCT,X,A,F,PDER
    ...
    if a[5] ne 0.0 then begin
        ...
        zj = REPLICATE(FIX(100, TYPE=SIZE(x,/TYPE)), nx)
        ezj = zj*0
    endelse
    ...
    case n of
        ...
        8:  F = A[0]*EZ + A[3]*EZj + A[6] + A[7]*X  ;JWJ edit
    ENDCASE
...
;
    ...
    PDER[*,2] = PDER[*,1] * Z
    PDER[*,3] = EZj
    if a[5] ne 0. then PDER[*,4] = A[3] * EZj * Zj/A[5]
    PDER[*,5] = PDER[*,4] * Zj
...
;-
;
Function DblGaussfit_JWJmod, x, y, a, $
    CHISQ=chisq, $
    ESTIMATES = est, $
    ...
    if nt lt 8 or nt gt 8 then $
       message,'NTERMS must have values from 8 for modified version.'
    ...
    yfit = CURVEFIT(x,y,weights,a,sigma, $
        ...
        FUNCTION_NAME = "DBLGAUSS_FUNCT") ;call curvefit
