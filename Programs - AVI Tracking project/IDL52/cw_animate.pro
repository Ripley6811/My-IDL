@movie_io
; $Id: cw_animate.pro,v 1.27 1998/10/06 20:23:36 alan Exp $
...
FUNCTION CW_ANIMATE_EV, event
   ...
   CASE event.id OF
      ...
      state.wMPEGButton : BEGIN
         ...
         END
; Avi_start
      state.wAVIButton : BEGIN
         aviFilename=DIALOG_PICKFILE(/WRITE)
         IF STRLEN(aviFilename) GT 0 THEN BEGIN
            tmpBase=WIDGET_BASE(TITLE='Animation: Creating AVI file', $
                       /FLOATING, MODAL=WIDGET_INFO(event.top, /MODAL), $
                       GROUP=event.top, TLB_FRAME_ATTR=27, /FRAME)
            tmpMsg = 'Writing AVI data to the file:'
            tmpText=WIDGET_TEXT(tmpBase, $
               XSIZE= ( STRLEN(tmpMsg) > STRLEN(aviFilename) )+5, $
               YSIZE=4, $
               /ALIGN_CENTER, $
               VALUE=['', tmpMsg, aviFilename])
            WIDGET_CONTROL, tmpBase, /REALIZE
;            WIDGET_CONTROL, /HOURGLASS
            aviID=AVI_OPENW(aviFilename, [state.sizex,state.sizey], RATE=1/state.delay, /TRUE)
            saveWindow = !D.WINDOW
            DEVICE, GET_VISUAL_NAME=visualName
                    FOR i=0, N_ELEMENTS((*state.pwinHdl))-1 DO BEGIN
                        IF (*state.pwinHdl)[i] GT -1 THEN BEGIN
...
function CW_ANIMATE, parent, sizex, sizey, nframes, $
  ...
  COMMON BitmapButtons
   ...
   wAVIButton = WIDGET_BUTTON(wButtonBase, VALUE='Write AVI')
   if (LMGR(/DEMO)) then WIDGET_CONTROL, wMPEGButton, SENS=0
   ...
   WIDGET_CONTROL, wTopBase, SET_UVALUE = $
      { wEndAnimationButton: wEndAnimationButton, $      ; End button
        ...
        wAVIButton: wAVIButton, $                        ; AVI button
