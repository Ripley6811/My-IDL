 ...
 */
#if defined(ALPHA_OSF) || defined(SUN_64) || defined(LINUX_ALPHA) || defined(LINUX_X86_64) || defined(HPUX_64) || defined(IRIX_64) || defined(AIX_64)
#define IDL_LONG_NOT_32		/* Can be used to configure code */
#endif
...
typedef enum {
  ...
  , IDL_MSG_SYSCODE_WINSOCK=3	/* Win32 winsock error codes
				   WSAGetLastError()*/
#endif
} IDL_MSG_SYSCODE_T;
...
#define IDL_STATIC_STRING(lexstr) { sizeof(lexstr)-1, 0, lexstr }
...
#if defined(LINUX_ALPHA) || defined(HPUX_64) ||	(defined(__cplusplus) && defined(WIN32)) || defined(darwin) || defined(linux)
 ...
 */
#define IDL_VERSION_STRING "6.1.1"
...
typedef struct {
  ...
  IDL_INT memory_bits;		/* # of bits used to address memory */
  IDL_INT file_offset_bits;	/* # of bits used to represent file offsets */
} IDL_SYS_VERSION;
...
extern int IDL_CDECL IDL_Main(int init_options, int argc, char *argv[]);
...
extern void IDL_CDECL IDL_MessageVa(int code, int action, va_list *args);
extern void IDL_CDECL IDL_MessageFromBlock(IDL_MSG_BLOCK block, int code,
        int action,...);
extern void IDL_CDECL IDL_MessageFromBlockVa(IDL_MSG_BLOCK block, int
        code, int action, va_list *args);
...
extern int IDL_CDECL IDL_AddToQueue(char* pString);
extern int IDL_CDECL IDL_GetWait(int fType);
extern int IDL_CDECL IDL_SetWait(int fType, int iVal);
